...
@endphp
                    ...
                    <td><span class="badge bg-secondary"> {{ $status->status_konfirmasis->status_konfirmasi}}</span></td>

                    @elseif($status->status_konfirmasis_id== 2)
                    <td><span class="badge bg-success"> {{ $status->status_konfirmasis->status_konfirmasi}}</span></td>

                    @elseif($status->status_konfirmasis_id== 3)
                    <td><span class="badge bg-danger">{{ $status->status_konfirmasis->status_konfirmasi}}</span> </td>
                    @endif
                    <td> 

                        <!--STATUS DI SETUJUI -->
                        <a href="/status_setuju/{{ $data->kode_peminjaman }}" type="button" class="btn btn-success"><i class="bi bi-check-lg"></i></a></br>
                        <!--STATUS DI TOLAK-->
                        <a href="/status_ditolak/{{ $data->kode_peminjaman }}" type="button" class="btn btn-danger"><i class="bi bi-x"></i></a>
